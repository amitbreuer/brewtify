...
test('interactive demo renders host, guest and direct start with no API traffic', async () => {
  ...
  const invite = page.locator('details').filter({ has: page.locator('summary', { hasText: 'Invite friends' }) });
  const inviteToggle = invite.locator('summary');
  assert.equal(await invite.evaluate(element => element.open), false);
  assert.equal(await page.getByLabel('Telegram invitation', { exact: true }).isVisible(), false);
  await inviteToggle.focus();
  await inviteToggle.press('Enter');
  assert.equal(await invite.evaluate(element => element.open), true);
  await page.getByAltText('QR code opening this party invitation in Telegram').waitFor();
  assert.equal(await page.getByLabel('Telegram invitation', { exact: true }).isVisible(), true);
  assert.equal(await page.getByRole('button', { name: 'Copy invite', exact: true }).isVisible(), true);
  await inviteToggle.press('Space');
  assert.equal(await invite.evaluate(element => element.open), false);
  ...
  assert.equal(await page.locator('summary', { hasText: 'Invite friends' }).count(), 0);
  assert.equal(await page.getByLabel('Telegram invitation', { exact: true }).count(), 0);
